...
label para_3:
    ...
    menu:
        ...
        "Столовая":
            ...
            m "Но... Здесь я вновь обнаружил её, крылатый символ вшэ." 
            m "Не хотелось искать себе еще приключений на голову, но выбора не осталось, и я решил подсесть к ней."
            main "Привет."
            crow "..."
            main "Эм, расскажешь о себе?"
            crow "..."
            m "Признаю, мои навыки коммуникации далеко не на высоком уровне." 
            m "Но, я думаю, здесь дело не во мне, и она просто не умеет разговаривать…"
            m "Между тем, пришло время возвращаться на пары."
            jump para_4
    return
label para_4:
...
label bus_stop:
    ...
    menu: 
        "Отправиться на поиски другой остановки":
            m "Думаю, другого выбора нет. {w}И… Кажется, я вижу несколько знакомых лиц."
            menu:
                "Подойти к студенту":
                    main "Прив..."
                    geo "Так или иначе мы уже виделись."
                    main "Тоже пытаешься покинуть это место?"
                    geo "Да, и пока безуспешно. {w}Я думаю дойти до метро и не думаю, что у тебя есть выбор лучше, чем пойти со мной."
                    main "Пожалуй, ты прав…"
                "Подойти к вороне":
                    m "И вновь я встречаю ее в неожиданных местах…"
                    main "Давно не виделись."
                    if crow==1:
                        main "И спасибо тебе за сегодня!"
                    crow "..."
                    m "Говорить она не начала, но я чувствую, что что-то изменилось"
                    m "Она начала уходить, попутно оглядываясь на меня. {w}Будто бы она хочет, чтобы я пошел за ней…"
                    m "Она привела меня в сторону метро. {w}Наверное, сейчас это мой единственный способ отсюда выбраться…"
                    if crowroute<3:
                        jump metro 
                    if crowroute>=3:
                        jump end
...
label metro:
    ...
    return
label end:
    if (good>=bad) and avtaz<3 and crowroute<3:
        scene end with fade
    ...
    if avtaz>=3:
        ...
        menu: 
            ...
            "Вмешаться":
                ...
                return
    if crowroute>=3:
        m "Но хочу ли я выбираться отсюда…?"
        m "Ворона почувствовала то же самое, что и я. {w}Она пошла дальше и я последовал за ней."
        m "..."
        m "Кажется, я знаю это место…"
        scene strigino with fade
        m "Но почему? {w}Вдруг я начал кое-что понимать…"
        m "На взлет отправляется самолет Нижний Новгород - Москва. По моему лицу начинают течь слёзы."
        m "Ворона машет мне на прощание, расправляет крылья и улетает вслед за самолетом…"
        main "Прощай! Надеюсь там ты обретёшь свое счастье…"
        return
